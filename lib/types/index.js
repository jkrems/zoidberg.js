...
function TypeSystem() {
  ...
  var ArrayType = types.ArrayType = new ParametricType('Array');
  ...
  function unify(target, other, oneWayOnly) {
    ...
    if (target instanceof TypeVariable) {
      ...
      if (oneWayOnly) {
        throw new Error(util.format('Can not unify specific %s with generic %s', target, other));
      }
  ...
  // the node's data type could be used everywhere type is expected,
  // e.g. a node of type String can be used when a String is expected,
  //      a node of type $a can be used anywhere
  //      a node of type Function($a, $a) can be used when a Function(String, String)
  //                                      but not when a Function(String, Int) is expected.
...
TypeReference.prototype.toString = function toString() {
  ...
  var params = types.length ? ('(' + types.map(prettyPrint).join(', ') + ')') : '';
