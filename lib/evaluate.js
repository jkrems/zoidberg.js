...
function evaluate(program, bindings, node) {
  ...
  switch (node.type) {
    ...
    case 'MatchExpression':
      var targetValue =
        node.target ?
        evaluate(program, bindings, node.target) : true;
      ...
      if (!foundMatch) {
        throw new Error('No match for ' + targetValue + ' found! ' +
          JSON.stringify(node.cases));
      }
      return matchEval;
    ...
    case 'EnumExpression':
      _.each(body.constructors, function(ctor) {
        ...
        ctor.body = new ZB.ArrayExpression(null, [
          new ZB.LiteralExpression(
            null, typeName + '.' + ctor.name, bindings.String)
