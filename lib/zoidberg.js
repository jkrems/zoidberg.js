...
function MulExpression(state) {
  var rootNode = ValueExpression(state);
...
function runFunction(program, fn, args) {
  var bindings = fn.params.reduce(function(bound, param, idx) {
    bound[param.name] = args[idx];
    return bound;
  }, {});

  return evaluate(program, bindings, fn.body);
}
...
function runMain(program, argv) {
  ...
  var result = runFunction(program, mainFn, parsedArgs);
