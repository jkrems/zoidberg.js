...
function IdentifierExpression(state) {
  ...
  var name = Identifier(state);
  var loc = getLocation();
  return new ZB.IdentifierExpression(loc, name);
}

function ArrayExpression(state) {
  var getLocation = state.trackNodeLocation();
  state.readString('[');

  var items = [];

  var firstItem = state.try(function(s) {
    s.skip(Whitespace);
  ...
  if (firstItem !== undefined) {
    ...
    // read element, followed by whitespace
    do {
      var item = state.try(function(s) {
        s.skip(Whitespace);
        s.readString(',');
        s.skip(Whitespace);
        return ListExpressionItem(s);
  ...
  state.readString(']');
  var loc = getLocation();
  return new ZB.ArrayExpression(loc, items);
}

function ValueExpression(state) {
  return state.or(ArrayExpression, LiteralExpression, IdentifierExpression, ParenExpression);
}
...
function AddExpression(state) {
  ...
  do {
    var node = state.try(function(s) {
      ...
      var loc = getLocation();
      return new ZB.BinaryExpression(loc, op, rootNode, right);
    });
    if (node !== undefined) {
      rootNode = node;
    }
...
function MatchElseCondition(state) {
  ...
  state.readString(':');
  var loc = getLocation();
  return new ZB.LiteralExpression(loc, true, Types.BoolType);
}
...
function Declaration(state) {
  ...
  if (params === undefined) {
    ...
    return new ZB.FunctionDeclaration(loc, name, _.pluck(params, 'name'), body,
